...
func (menuService *MenuService) getBaseMenuTreeMap() (err error, treeMap map[string][]system.SysBaseMenu) {
	...
	err = global.TPA_DB.Order("sort").Preload("Parameters").Find(&allMenu).Error
...
func (menuService *MenuService) AddMenu(menu system.SysBaseMenu) error {
	if !errors.Is(global.TPA_DB.Where("name = ?", menu.Name).First(&system.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("已存在name,请修改name")
	}
	return global.TPA_DB.Create(&menu).Error
}
...
func (menuService *MenuService) DeleteMenu(id uint) (err error) {
	err = global.TPA_DB.Preload("Parameters").Where("parent_id = ?", id).First(&system.SysBaseMenu{}).Error
	if err != nil {
		var menu system.SysBaseMenu
		db := global.TPA_DB.Preload("SysAuthoritys").Where("id = ?", id).First(&menu).Delete(&menu)
		err = global.TPA_DB.Delete(&system.SysBaseMenuParameter{}, "sys_base_menu_id = ?", id).Error
		if err != nil {
			return err
		}
		if len(menu.SysAuthoritys) > 0 {
			err = global.TPA_DB.Model(&menu).Association("SysAuthoritys").Delete(&menu.SysAuthoritys)
		} else {
			err = db.Error
			if err != nil {
				return
...
func (menuService *MenuService) getMenuTree(authorities []system.SysAuthority) (err error, treeMap map[string][]system.SysMenu) {
	...
	err = global.TPA_DB.Where("authority_id in (?)", authorityIDs).Order("sort").Preload("Parameters").Find(&allMenus).Error
	if err != nil {
		return
	}
	//list去重
...
func (menuService *MenuService) GetMenuAuthority(info *request.GetAuthorityId) (err error, menus []system.SysMenu) {
	err = global.TPA_DB.Where("authority_id = ? ", info.AuthorityId).Order("sort").Find(&menus).Error
...
func (menuService *MenuService) GetBaseMenuById(id uint) (err error, menu system.SysBaseMenu) {
	err = global.TPA_DB.Preload("Parameters").Where("id = ?", id).First(&menu).Error
	return
}
