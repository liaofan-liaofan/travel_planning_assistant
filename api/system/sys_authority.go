...
func (a *AuthorityApi) CreateAuthority(c *gin.Context) {
	...
	if r.Level < dataScope.GetMaxLevel(curUser.Authority) {
		global.TPA_LG.Error("无权创建该角色!", utils.GetRequestID(c))
	...
	if r.DataScope == "自定义" {
		for _, v := range r.DeptId {
			depts = append(depts, system.SysDept{
				TPA_MODEL: global.TPA_MODEL{ID: v},
			})
		}
	}
...
func (a *AuthorityApi) DeleteAuthority(c *gin.Context) {
	...
	if err != nil {
		global.TPA_LG.Error("该角色不存在!", utils.GetRequestID(c))
...
func (a *AuthorityApi) UpdateAuthority(c *gin.Context) {
	...
	if curAuthority.Level < dataScope.GetMaxLevel(curUser.Authority) {
		global.TPA_LG.Error("更新失败, 当前用户无权修改该角色!", utils.GetRequestID(c))
		response.FailWithMessage("更新失败, 当前用户无权修改该角色!", c)
		return
	}
	//判断改级别是否高于当前用户级别
	if r.Level < dataScope.GetMaxLevel(curUser.Authority) {
		global.TPA_LG.Error("更新失败, 修改级别高于当前用户级别!", utils.GetRequestID(c))
	...
	if err, authority := authorityService.UpdateAuthority(authority, r.DeptId); err != nil {
		global.TPA_LG.Error("更新失败!", zap.Any("err", err), utils.GetRequestID(c))
