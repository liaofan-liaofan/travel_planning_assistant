...
func (b *BaseApi) Register(c *gin.Context) {
	...
	for _, v := range r.AuthorityIds {
		if err, authority := authorityService.GetAuthorityBasicInfo(system.SysAuthority{
			...
			global.TPA_LG.Error("注册失败, 角色不存在!", utils.GetRequestID(c))
	...
	user := &system.SysUser{TPA_MODEL: global.TPA_MODEL{CreateBy: curUser.ID, UpdateBy: curUser.ID}, Username: r.Username, NickName: r.NickName, Password: r.Password, Phone: r.Phone, Email: r.Email, Authorities: authorities, DeptId: r.DeptId}
	//数据权限校验
	canDo := dataScope.CanDoToTargetUser(curUser, []*system.SysUser{user})
	if !canDo {
		global.TPA_LG.Error("注册失败, 无权注册该用户!", utils.GetRequestID(c))
		response.FailWithMessage("注册失败, 无权注册该用户!", c)
		return
	}
	err, userReturn := userService.Register(*user)
	if err != nil {
		global.TPA_LG.Error("注册失败!", zap.Any("err", err), utils.GetRequestID(c))
...
func (b *BaseApi) GetUserList(c *gin.Context) {
	...
	if err, list, total := userService.GetUserInfoList(pageInfo, deptId, isAll); err != nil {
		global.TPA_LG.Error("获取失败!", zap.Error(err), utils.GetRequestID(c))
...
func (b *BaseApi) UpdatePassword(c *gin.Context) {
	...
	if err, _ := userService.UpdatePassword(u, user.NewPassword); err != nil {
		global.TPA_LG.Error("修改失败", zap.Error(err), utils.GetRequestID(c))
...
func (b *BaseApi) ResetPassword(c *gin.Context) {
	...
	u := &system.SysUser{
		TPA_MODEL: global.TPA_MODEL{ID: reqId.ID},
	}
...
func (b *BaseApi) GetUserInfo(c *gin.Context) {
	...
	if err, userInfo := userService.GetUserInfo(uuid); err != nil {
		global.TPA_LG.Error("获取用户信息失败", zap.Error(err), utils.GetRequestID(c))
...
func (b *BaseApi) SetUserInfo(c *gin.Context) {
	...
	if err != nil {
		global.TPA_LG.Error("修改失败!", zap.Any("err", err), utils.GetRequestID(c))
		response.FailWithMessage("操作用户不存在", c)
		return
	}
	canDo := dataScope.CanDoToTargetUser(curUser, []*system.SysUser{updateUser})
	if !canDo {
		global.TPA_LG.Error("修改失败, 无权修改该用户!", utils.GetRequestID(c))
	...
	for _, authorityId := range reqUser.AuthorityIds {
		if err, authority := authorityService.GetAuthorityBasicInfo(system.SysAuthority{AuthorityId: authorityId}); err != nil {
			global.TPA_LG.Error("设置角色不存在!", utils.GetRequestID(c))
	...
	if dataScope.GetMaxLevel(updateAuthorities) < dataScope.GetMaxLevel(curUser.Authority) {
		global.TPA_LG.Error("设置角色级别高于当前用户级别!", utils.GetRequestID(c))
		response.FailWithMessage("设置角色级别高于当前用户级别!", c)
		return
	}
	if err := userService.SetUserAuthorities(*updateUser, reqUser.AuthorityIds); err != nil {
		global.TPA_LG.Error("用户角色修改失败!", zap.Any("err", err), utils.GetRequestID(c))
		response.FailWithMessage("用户角色修改失败", c)
		return
	}
	user := system.SysUser{TPA_MODEL: global.TPA_MODEL{ID: reqUser.ID, UpdateBy: curUser.ID}, DeptId: reqUser.DeptId, Phone: reqUser.Phone, Email: reqUser.Email, NickName: reqUser.NickName, HeaderImg: reqUser.HeadImg, UUID: reqUser.UUID}
	if err, sysUser := userService.SetUserInfo(user); err != nil {
		global.TPA_LG.Error("设置失败", zap.Error(err), utils.GetRequestID(c))
...
func (b *BaseApi) SetUserDept(c *gin.Context) {
	...
	curUser := utils.GetUser(c)
	user := system.SysUser{TPA_MODEL: global.TPA_MODEL{ID: reqUser.ID, UpdateBy: curUser.ID}, DeptId: reqUser.DeptId, UUID: reqUser.UUID}
...
func (b *BaseApi) DeleteUser(c *gin.Context) {
	...
	if err != nil {
		global.TPA_LG.Error("删除失败, 该用户不存在!", utils.GetRequestID(c))
	...
	if !canDo {
		global.TPA_LG.Error("删除失败, 无权删除该用户!", utils.GetRequestID(c))
		response.FailWithMessage("删除失败, 无权删除该用户!", c)
		return
	}
	//删除用户
	if err := userService.DeleteUser(reqId.ID); err != nil {
		global.TPA_LG.Error("删除失败!", zap.Any("err", err), utils.GetRequestID(c))
...
func (b *BaseApi) ImportExcel(c *gin.Context) {
	...
	if err != nil {
		global.TPA_LG.Error("接收文件失败!", zap.Error(err), utils.GetRequestID(c))
		response.FailWithMessage("接收文件失败", c)
		return
	}
	_ = c.SaveUploadedFile(header, global.TPA_CONFIG.Excel.Dir+"ExcelImport.xlsx")
...
func (b *BaseApi) LoadExcel(c *gin.Context) {
	...
	if err != nil {
		global.TPA_LG.Error("加载数据失败", zap.Error(err), utils.GetRequestID(c))
...
func (b *BaseApi) ExportExcel(c *gin.Context) {
	...
	filePath := global.TPA_CONFIG.Excel.Dir + excelInfo.FileName
	if err := userService.ParseDataListToExcel(excelInfo.InfoList, filePath); err != nil {
		global.TPA_LG.Error("导出excel失败", zap.Error(err), utils.GetRequestID(c))
...
func (b *BaseApi) DownloadTemplate(c *gin.Context) {
	...
	filePath := global.TPA_CONFIG.Excel.Dir + name
	if err := userService.Template(filePath); err != nil {
		global.TPA_LG.Error("模板下载失败", zap.Error(err), utils.GetRequestID(c))
